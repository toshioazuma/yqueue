//
//  CustomerSlipOptionAlertVC.swift
//  YQueue
//
...
class CustomerSlipOptionAlertVC: AlertVC {
    ...
    override func viewDidLoad() {
        ...
        tableViewTopOffset.constant = optionLabel.convert(optionLabel.frame, to: view).origin.y - optionLabel.frame.size.height*1.5
        
        var models = Array<CustomerMenuOptionAlertVM>()
        for option in options {
            let model = CustomerMenuOptionAlertVM(option: option, tap: { [weak self] in
                guard let `self` = self else {
                    return
                }
    ...
    static func show(in vc: UIViewController, withOptions options: Array<MenuItem.Option>, optionLabel: UILabel, selectionCallback: @escaping (MenuItem.Option) -> Void, offsetCallback: @escaping (CGFloat) -> Void) -> CustomerSlipOptionAlertVC {
        let alert = Storyboard.slipOptionAlert()
