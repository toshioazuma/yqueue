...
class CustomerChangePaswordVC: BaseVC {
    
    @IBOutlet weak var currentPassword: CustomerChangePasswordInput!
    @IBOutlet weak var newPassword: CustomerChangePasswordInput!
    @IBOutlet weak var repeatNewPassword: CustomerChangePasswordInput!
    ...
    override func viewDidLoad() {
        ...
        form.onSubmit(with: saveButton).observeValues { [weak self] in
            ...
            Api.auth.changePassword(from: $0[0], to: $0[1]).observe(on: QueueScheduler.main).observe { [weak self] in
